...
A backup stops the node, copies `nom`, `network`, `consensus` and `cache` from the data directory into a staging area, restarts the node, and only then compresses the copy into `/backup/go-zenon_backup_<MM-DD-YY_HHMMSS>.tar.gz` with a sha256 sidecar (`.hash`). Archives beyond `--max-backups` (default 7, maximum 30) are pruned, oldest first.
...
If copying fails, the node is restarted before the error is reported.
